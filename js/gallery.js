...
FgGallery.prototype.generateItems = function () {
    ...
    this.gallerySelector.forEach(function (galleryContainer, i) {
        ...
        galleryContainer.querySelectorAll('img').forEach(function (imageItems, key) {
            ...
            var newImages = '';
            newImages += `<div class="gallery-items" style="background: url(${imageItems.src.replace(this.imgReg, 'big')}) center / cover; "></div>`
            ...
            newImages.onclick = function () {
                ...
                This.activeImage.src = This.activeImage.src.replace(This.imgReg, 'big');
...
FgGallery.prototype.next = function () {
    this.nextBtn.onclick = () => {
        ...
        if (this.galleryItemSequence < this.originalImages[`gallery${this.gallerySequence}`].length) {
            this.activeImage.src = this.originalImages[`gallery${this.gallerySequence}`][this.galleryItemSequence].src.replace(this.imgReg, 'big');
        } else {
            this.galleryItemSequence = 0
            this.activeImage.src = this.originalImages[`gallery${this.gallerySequence}`][0].src.replace(this.imgReg, 'big');
        }
    }
}
